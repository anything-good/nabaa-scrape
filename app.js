...
app.post('/upload', upload.single('csvfile'), async (req, res) => {
  ...
  const listRes = await fetch('https://ali.elitestudents.link/api/laptop-lists', {
    method: 'POST',
    headers: {
  ...
  fs.createReadStream(filePath)
    ...
    .on('data', async (row) => {
      ...
      let privatePrice = row.price;
      let { publicName, publicPrice } = await scrape(row.name);

      await fetch(`https://ali.elitestudents.link/api/laptop-lists/${list.id}/laptops`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer rKtFskRVvRK9J1r0X5xMGnqU2NUvMC2JcOtXtLVt'
